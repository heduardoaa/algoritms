...
function distributeGifts(packOfGifts, reindeers) {
    if (!Array.isArray(packOfGifts) && !Array.isArray(reindeers)) {
        throw new Error("packOfGifts or reindeer's should be an array");
    }
    let weightAllowed = 0;
    for(let name of reindeers) {
        weightAllowed += name.length * 2;
    }
    ...
    console.log(weightAllowed / weightGifts);
    const total = weightAllowed / weightGifts;

    return Number.isInteger(total) ? total : (Math.round(total) <= 1 ? Math.round(total) : Math.round(total) -1);
}

console.log(distributeGifts(['videogames', 'console'], ['midu'])) // 2